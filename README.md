...
在主窗口左侧拖动项目可调整优先顺序，也可右键选择“上移 / 下移”。顺序自动保存并同步到菜单栏；搜索时清空搜索后再排序。

菜单栏各项目下方直接显示可打开的本机服务链接（含自动发现的地址），点击即可跳转浏览器，无需打开项目窗口。多前端或管理后台可分别打开；未监听或被其他进程占用的地址不显示。自动发现只能识别端口，不能区分前端和 API，可在启动配置中填写服务名称与精确页面路径。
...
检查程序不依赖 XCTest，可在只有 Command Line Tools 的 Mac 上运行；端口回归检查使用 `/usr/bin/python3` 的标准库临时 HTTP 服务，应用本身不依赖 Python。共 11 组检查，覆盖项目排序和持久化回退、浏览器链接筛选、只读发现、医院 API/网页区分、大输出管道与超时、中文目录、整组进程停止、后台进程归属与启停、配置持久化、动态端口发现、跨项目同端口误高亮、IPv4/IPv6 同端口和共享监听、日志轮转与 ANSI 清理。不会启动真实业务项目。
